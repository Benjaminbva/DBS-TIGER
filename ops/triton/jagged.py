...
class PaddedToJaggedTensor(Function):
    ...
    def forward(ctx, x: Tensor, lengths: Tensor, max_len: int) -> NestedTensor:
        ...
        B, N, D = x.shape
        mask = torch.arange(max_len, device=x.device).unsqueeze(0).repeat(
            x.shape[0], 1
        ) < lengths.unsqueeze(1)
        ...
        target = torch.nested.nested_tensor(
            [[]], layout=torch.jagged, device=x.device, requires_grad=x.requires_grad
        )
...
def _padded_to_jagged_kernel(
    ...
    jagged_row_start_offset, jagged_row_end_offset = tl.load(
        offsets_ptr + row_group
    ), tl.load(offsets_ptr + row_group + 1)
    ...
    out_values_tile_start = (
        out_values_ptr
        + (jagged_row_start_offset + col_group * BLOCK_SIZE_N) * out_values_stride_B
    )
